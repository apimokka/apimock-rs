# SUMMARY

- [Home](./README.md)

---

- [Getting started](./getting-started/README.md)
    - [Install and first response](./getting-started/install-and-first-response.md)
    - [Your first config file](./getting-started/your-first-config-file.md)
    - [Your first rule](./getting-started/your-first-rule.md)

---

- [Guides](./guides/README.md)
    - [Serve JSON files from a folder](./guides/serve-json-files-from-a-folder.md)
    - [Match on URL path and method](./guides/match-on-url-path-and-method.md)
    - [Match on headers](./guides/match-on-headers.md)
    - [Match on the request body](./guides/match-on-the-request-body.md)
    - [Return errors and status codes](./guides/return-errors-and-status-codes.md)
    - [Vary the response for one path](./guides/vary-the-response-for-one-path.md)
    - [Simulate slow or flaky backends](./guides/simulate-slow-or-flaky-backends.md)
    - [Serve over HTTPS](./guides/serve-over-https.md)
    - [Reload TLS certificates without restart](./guides/reload-tls-certificates-without-restart.md)
    - [Script with Rhai middleware](./guides/script-with-rhai-middleware.md)
    - [Filter the served file tree](./guides/filter-the-served-file-tree.md)
    - [Add or change a rule](./guides/add-or-change-a-rule.md)
    - [Check what a request returns](./guides/check-what-a-request-returns.md)
    - [Validate config in CI](./guides/validate-config-in-ci.md)
    - [Dry-run a rule](./guides/dry-run-a-rule.md)
    - [Watch matches live](./guides/watch-matches-live.md)
    - [Migrating to 6.0.0](./guides/migrating-to-6-0.md)
    - [Migrating to 6.1.0](./guides/migrating-to-6-1.md)

---

- [Reference](./reference/README.md)
    - [`apimock.toml` root settings](./reference/apimock-toml-root-settings.md)
    - [Rule-set schema](./reference/rule-set-schema.md)
    - [Operator reference](./reference/operator-reference.md)
    - [Body path syntax](./reference/body-path-syntax.md)
    - [CLI reference](./reference/cli-reference.md)
    - [Response headers](./reference/response-headers.md)
    - [Threat model](./reference/threat-model.md)

---

- [Using apimock as a library](./library/README.md)
    - [Crates and architecture](./library/crates-and-architecture.md)
    - [Editing a configuration](./library/editing-configuration.md)
    - [Running a mock server](./library/running-a-mock-server.md)
    - [API stability](./library/api-stability.md)
    - [Known limitations](./library/known-limitations.md)

---

- [How it works](./how-it-works/README.md)
    - [Matching order and precedence](./how-it-works/matching-order-and-precedence.md)
    - [Response decision flow](./how-it-works/response-decision-flow.md)
    - [Architecture](./how-it-works/architecture.md)
    - [The workspace and its crates](./how-it-works/workspace.md)
    - [Design notes](./how-it-works/design-notes.md)
    - [Performance](./how-it-works/performance.md)

---

- [Contributing](./contributing/README.md)
    - [Build and test locally](./contributing/build-and-test-locally.md)
    - [The quality gates](./contributing/the-quality-gates.md)
    - [The RFC process](./contributing/the-rfc-process.md)
    - [Vision and goals](./contributing/vision-and-goals.md)
