...
impl RuleOp {
    ...
    pub fn is_match(&self, text: &str, checker: &str) -> bool {
        match self {
            ...
            Self::Regex         =>  regex::Regex::new(checker).map(|re| re.is_match(text)).unwrap_or(false),
            Self::NotRegex      => !regex::Regex::new(checker).map(|re| re.is_match(text)).unwrap_or(false),
        }
    }
