...
fn add_header_condition_creates_condition() {
    ...
    assert!(
        ...
        "should return changed node ids"
    );
    ...
    assert!(
        headers.0.contains_key("x-new-header"),
...
fn remove_body_condition_deletes_it() {
    ...
    let add_result = ws
        .apply(EditCommand::AddBodyCondition {
            rule_id,
            condition: crate::view::BodyConditionPayload {
                kind: crate::view::BodyConditionKind::Json,
    ...
    ws.apply(EditCommand::RemoveBodyCondition { id: cond_id })
        .expect("remove");
}
