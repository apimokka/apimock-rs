...
use crate::{view::EditCommand, workspace::Workspace};
...
fn snapshot_default_filter_hides_dotfiles_and_target() {
    ...
    assert!(
        !names.contains(&".env"),
        ".env must be hidden by default filter"
    );
    assert!(
        !names.contains(&"target"),
        "target/ must be hidden by default filter"
    );
}
...
fn snapshot_show_hidden_exposes_dotfiles() {
    ...
    assert!(
        names.contains(&".env"),
        ".env should be visible when show_hidden=true"
    );
}
...
fn snapshot_include_filter_shows_only_matching_files() {
    ...
    let names: Vec<&str> = tree.entries.iter().map(|e| e.name.as_str()).collect();
    assert!(
        names.contains(&"users.json"),
        "json file should be included"
    );
    assert!(
        !names.contains(&"schema.toml"),
        "toml file should be excluded by include filter"
    );
}
