...
- **Major version.** Whether and when 6.0.0 happens remains the project
  ...
  **Amended 2026-08-17:** v6's *concept* is now accepted
  ([RFC 048](./rfcs/done/048-v6-cli-interface-concept.md)), and the
...
| 040 | [Trace channel: header redaction](./rfcs/done/040-trace-capture-and-redaction.md) | P1 | — | **Accepted** 2026-08-17 — implemented on `main`, awaiting 6.0.0. **Goal 3 removed** — see 050 |
| 050 | [Should non-JSON bodies be captured at all?](./rfcs/done/050-non-json-body-capture-decision.md) | P2 | 040 | **Accepted** — decided 2026-08-17, answer (2), presence only. Implemented on `main`, awaiting 6.0.0 |
| 051 | [Redact credential headers in verbose logging](./rfcs/done/051-verbose-log-header-redaction.md) | **P1** | 040 | **Accepted** 2026-08-17 — security. Implemented on `main`, awaiting 6.0.0. Was attempted **without new public fields** so it could ship in a true minor; it did not make 5.19.0, so it now ships with the rest |
| 052 | [`#[non_exhaustive]` on public types](./rfcs/done/052-non-exhaustive-public-types.md) | P1 | — | **Accepted** — decision approved 2026-08-17. Implemented on `main`; breaking, ships at the v6 boundary |
| 041 | [Error type shape: boxing, `kind()`, `#[non_exhaustive]`](./rfcs/done/041-error-type-shape.md) | P2 | — | **Accepted and handed off 2026-08-20; amended the same day to cover the whole public surface.** Breaking, so 6.0.0. 15 lint suppressions across 8 files, caused by `toml::de::Error` (88 bytes) held by value in two parse variants; plus `#[non_exhaustive]` on ~43 re-exported types, which **closes R-09** |
| 042 | [~~`sync_from_disk` incremental reconciliation~~ → **external change detection**](./rfcs/done/042-external-change-detection.md) | P2 | — | **Drafted 2026-08-19** — now a *correctness* RFC, not a feature one: `sync_from_disk`'s public doc promises NodeId preservation its own implementation contradicts three lines later, and `reseed_after_edit` was never built. **G1 answered 2026-08-17.** The owner rejects both automatic behaviour change *and* continuous watching, so no filesystem watcher and no `notify` dependency. Detection is a boot-time file list plus an existence/mtime poll; the response is to **ask the user**, not to act. That removes RFC 042's premise — it existed to make reconciliation *incremental* because wholesale reload was assumed too costly to do often, and a reload gated on explicit confirmation does not happen often. RFC 024 already covers part of the remainder |
| 043 | [Module split: `workspace/edit.rs`](./rfcs/done/043-module-split-edit-rs.md) | P2 | — | **Drafted 2026-08-19.** `server/trace.rs` **dropped from scope** — 570 code lines, not an outlier; its raw count was 44% tests. Sequence after RFC 057 |
...
[RFC 048](./rfcs/done/048-v6-cli-interface-concept.md): the CLI
...
| Finding | Disposition |
|---|---|
| **Independent external audit of 6.0.0 — 59 findings, 5 High, 0 Critical.** Run 2026-08-31 → 09-01 against `4c77bfb` by an external architect; report in `.git-exclude/tmp/audit/report/`, my review in `.git-exclude/reviewed/external-audit-2026-09-01/REVIEW-001.md`. **I re-ran every headline finding myself and all reproduced**: credentialed any-origin CORS reflection (S-01); a mistyped rule key passing `validate` and then matching *every* request (F-17); `round_robin` never rotating when a rule set serves more than one match group (F-01); the whole `AppState` deep-cloned per request because it holds `Config` by value (P-01). The audit judged us against our own stated scope, quoted the threat model back, credited honestly-documented gaps as honest, and raised **nothing** that contradicts a decision we had recorded | **RFCs [067](./rfcs/proposed/067-cors-credential-reflection.md)–[071](./rfcs/proposed/071-share-application-state.md) drafted 2026-09-01, awaiting owner approval.** Between them they close all five Highs plus F-17. Sequencing note: **067 and 068 are security/availability and should go first**; **069, 070 and part of F-04 are behaviour-breaking even though not API-breaking** — a config that loads today stops loading, a gating condition that passes starts failing, response bytes change — so they want a **minor release with a migration note, not a patch**. The medium/low tail is now drafted too — **[072](./rfcs/proposed/072-header-matching-fails-closed.md)–[079](./rfcs/proposed/079-dead-and-misleading-code.md)**, grouped by subsystem so each is one reviewable change: header matching failing closed; the trace channel's wrong events, false back-pressure docs and unredacted bodies; TLS handshake bounds and silent HTTP degradation; URL path decoding/case/prefix; serving JSON as written; the per-request work cluster; four false documentation statements plus a troubleshooting page; and the dead-code tail. **Thirteen RFCs now cover every finding in the audit.** Three ordering constraints are recorded in `rfcs/README.md` and are not preference: 075 must honour its decode-before-normalise rule or it reintroduces the traversal advisory by another route; 078 is partly blocked by 070/073/075/076 and its blocked half becomes mandatory if those are declined; 071 precedes 077 because it is ~20× the cluster behind it |
| **Audit finding F-06 (missing PATCH/HEAD/OPTIONS matching) cannot be fixed as recommended within 6.x.** The audit recommends adding `Patch`/`Head`/`Options` to `HttpMethod`. Verified 2026-09-01: `HttpMethod` is a **public enum, not `#[non_exhaustive]`**, and is in `crates/apimock-routing/public-api.txt`. Adding a variant breaks downstream exhaustive matching; so does adding `#[non_exhaustive]` to it now. Both routes are 6.x-incompatible and RFC 039's gate will surface either as a declared baseline change. The auditor could not have known — RFC 039 landed 2026-08-30, four days after their baseline commit | **Owner decision, and the first real test of the additive-only gate.** The gap is genuine: PATCH is unmatched and every reference implementation supports it. Options are (a) defer to the next incompatible release, (b) match methods by string alongside the enum so the enum need not change, or (c) decide the exhaustive-match breakage is acceptable and declare it. **(b) is worth costing before (a) is accepted** — it may close the user-visible gap with no API change at all, which would make this a 6.x fix after all |
| **No consumer is coming for the three unproven library surfaces — the GUI team excludes all of them by design.** `Workspace::has_external_changes`/`sync_from_disk`, the whole `apimock_server::control` module, and `TraceEmitter`'s subscription side have no caller anywhere in this repository. The 6.0.0 library handoff asked the apimokka team four questions about them. Their reply (2026-08-31, `.git-exclude/tmp/apimokka-to-apimock-2026-08-21-v6-module-usage.md`) is that they cannot answer any of the four, **structurally rather than as a deferral** — their programme explicitly defers *"real `Workspace` file I/O and persistence"*, *"helper subprocess start/stop/reload/restart control"*, *"live trace UDS/TCP connection"* and *"external-edit detection and conflict handling"*. They classify with `ReloadHint::for_key` but run no server, so never act on a hint | **Architect's.** Their own conclusion is the right one and worth adopting verbatim: *"your unproven surfaces are unproven partly because the only GUI consumer in view deferred all four by design … waiting on us will not resolve them — that is an argument for spiking them yourselves."* So the disposition I recorded — *first consumer will tell us* — is void; **either we exercise these ourselves or they stay unexercised indefinitely.** Cheapest honest option is a small integration test per surface, which would at least answer whether `sync_from_disk` preserves unsaved edits and how a `TraceEmitter` consumer subscribes. Until then `docs/src/library/known-limitations.md` must keep saying plainly that they are unproven, because nothing is going to change that on its own |
| **The public-API baselines are the documented tiebreaker and are absent from both places a consumer looks.** `docs/src/library/` told readers that `crates/*/public-api.txt` is authoritative when documentation and code disagree. Verified 2026-08-31: **not in the `6.0.0` tag** (RFC 039 landed at `1b7ebec`, after the tag at `c24117b`) and **not in any published crate tarball** (`exclude = ["public-api.txt"]`). They exist only on the default branch. Found by the apimokka team while sizing their adoption — they went looking for the tiebreaker and found nothing | **Documentation corrected 2026-08-31**; both pages now say where the baselines are and where they are not. **The compounding is mine and worth recording:** I endorsed the `exclude` in RFC 039's review as a good call the dev team was not asked to make — it is, a 4,300-line CI artefact does not belong in four tarballs — and then days later wrote documentation promising that excluded file as the authority, without connecting the two. Each decision was defensible; together they broke the promise. **Open question, not settled:** whether a crate should ship its own API baseline after all, since that is the one place it is pinned to the exact published version. Releases after 6.0.0 will at least carry them at the tag |
| **`apimock-server` and `apimock-routing` expose their entire internal module tree as public API — and RFC 039's gate now holds it stable.** Verified 2026-08-30: `apimock-server` **16 of 16** top-level modules are `pub mod`, `apimock-routing` **6 of 6** (`apimock` 3/4, `apimock-config` 5/6). So `apimock_server::http_util`, `::dyn_route`, `::response_handler`, `apimock_routing::util::glob` and the rest are surface a consumer may depend on. None reads as a designed external API; it reads as nothing ever having narrowed it. Found by the dev team while generating RFC 039's first baseline — the first time anyone has looked at the full public surface in one place | **Architect's — deferred, and it cannot be done in 6.x.** Narrowing `pub` → `pub(crate)` is a breaking change to a surface 6.0.0 just froze; RFC 039's gate would correctly refuse it. So it waits for whenever an incompatible release is actually on the table — which is not a decision anyone has taken, and this row is not an argument for taking it. **Immediate consequence worth knowing before RFC 043 starts:** a refactor that only *intends* to move code between internal modules will still produce a baseline diff on these two crates, because the module it moves out of is already public. RFC 039 § "Interaction with RFC 043" calls that a feature — the gate catching a leak of module structure — and it is; but it makes the first RFC 043 change a near-certain baseline update. Correctly **not** fixed inside the gate's own task: that would have been an undeclared API change on the commit adding the gate meant to catch undeclared API changes |
| **16 `rustdoc::broken_intra_doc_links` warnings in `apimock-routing`, and nothing in the project would catch them.** Intra-doc links such as `` [`Equal`] `` in `body_operator.rs` / `strategy.rs` that resolve to no item in scope. Surfaced only because RFC 039's baseline generation runs rustdoc; the gate's generation step discards stderr, correctly, so they do not affect it | **Architect's, low priority.** Not part of the gated public surface and harmless to the build — but they are broken links in published documentation, and they went unseen because **no job runs `cargo doc`**. A `cargo doc --no-deps` check (warnings-as-errors, or at least reported) would catch this class. Cheap; pairs naturally with the `mdbook-linkcheck` item already queued, since both are "published documentation contains links that do not resolve" |
| **`cargo package --workspace` cannot be an unconditional CI gate — it fires spuriously between releases.** Added as a `package` job in the post-6.0.0 release-process work, on a measurement taken at the one moment the workspace source was byte-identical to what is published (zero source changes since the 6.0.0 tag). **Proved otherwise 2026-08-28:** adding a `pub fn` to `apimock-routing` and calling it from `apimock-config`, with the version left at an already-published 6.0.0, fails verification — `error[E0425]: cannot find function … in crate apimock_routing`. `apimock-config`'s tarball resolves its sibling **from crates.io** once that version exists there; cargo's co-packaging temp registry is used only when the version is *not* published | **Rejected as merged; architect's to re-shape.** The gate is worth having — it caught a deliberate `compile_error!` break when tested — but unconditionally it turns red on the first cross-crate public change after a release and stays red until the next one, which is the gate-people-ignore outcome. **It must run only where the workspace version is not already on crates.io**: always true on the release path, never true between releases. My own handoff asserted this concern without recording how I had produced it (a 5.19.0 tree carrying unpublished RFC 065 changes under a published version number), so the dev team tested the claim as written and reasonably concluded it did not hold |
| **An unauthenticated `api.github.com` call sits inside a required CI job.** `docs` installs mdBook by `curl`-ing the GitHub releases API for the latest asset URL. It failed on the post-6.0.0 branch with `jq: error … Cannot iterate over null` — a transient rate-limit, correctly diagnosed by the dev team as pre-existing and not their change, and cleared by a rerun | **Architect's, low priority.** A required job that can fail for reasons unrelated to the commit trains people to rerun rather than read. Options: authenticate the call with the workflow's own `GITHUB_TOKEN`, pin the mdBook/mdbook-mermaid versions instead of resolving "latest" at runtime, or cache the binaries. Pinning is probably right — it also makes the docs build reproducible, which resolving "latest" on every run currently prevents |
| 🔴 **`npm-core-publish` races npm's registry propagation, and will lose again.** The job is `needs: npm-platforms-publish` with **no wait, no retry and no availability check**, then runs `npm install --package-lock-only` followed by `npm ci`. **npm silently omits `optionalDependencies` it cannot resolve**, so if `@apimock-rs/bin-*@X.Y.Z` has not propagated, the generated lock lacks all three, and `npm ci` rejects it — `EUSAGE`, *"package.json and package-lock.json are in sync"*. **Measured margin between the last platform publish and the core job starting: 5.19.0 4s ✅, 5.19.1 3s ✅, 4.8.2 3s ✅, 6.0.0 2s ❌.** The design has always allowed ~3 seconds; 6.0.0 is the first time npm was slower than that. The rerun, 113s later, succeeded | **Architect's, before the next release.** **It will recur** — nothing changed, so every release re-runs the same ~3-second gamble, and the eight prior successes were the race being won rather than absent. Consequence is a **partially published release**: on 6.0.0, three platform packages live, core absent, crates.io skipped. The fix is a pattern this same workflow already contains — `verify-published` retries `npm pack` six times over 90s — applied one job earlier: retry the lock/`ci` step, or poll until the three platform versions resolve. **Also: `RELEASING.md` documents partial-publish recovery for crates.io only**; the npm case has no written procedure, and the recovery used (`gh run rerun --failed`) should be recorded |
| **Two `tests/args.rs` tests prove less than a reader would assume.** `bare_relative_config_resolves_the_same_as_dot_slash_prefixed` and `config_equals_form_resolves_the_same_as_space_form` use a `[listener]`-only config fixture, which **fails to load** (`missing field "service"`) — the process exits 1. They pass anyway because they poll only for the `[config]` line, which apimock prints *before* the load fails. Found by the dev team hitting the same fixture in a new test during the `apimock serve` work; mechanism reproduced independently 2026-08-28 | **Architect's, after 6.0.0. Not a defect.** Both tests assert path *resolution*, which is what their names say and what they genuinely prove. The risk is narrow: anyone strengthening either one to check the server actually starts will hit a fixture that cannot start, and the reason is non-obvious. Recorded here because the dev team's note lives in a review package outside git and would be lost. **Fix when next touched:** swap in a minimal valid config (`[service]` with `fallback_respond_dir`) and `-p 0` |
| **The docs-build gate catches build failures, not content integrity.** Added by the pre-6.0.0 documentation work (audit item E-1) as a `docs` job in `ci.yaml`. The dev team tried to break it three ways and reported that two did not fail the build; I reproduced the load-bearing one: a `SUMMARY.md` entry pointing at a nonexistent page makes **mdbook silently create a stub and exit 0**. Separately, **mdbook does not validate internal anchors** — a broken `#…` link in `return-errors-and-status-codes.md` survived until a human read the paragraph. So a forgotten page file and a broken cross-reference both pass the gate | **Architect's, after 6.0.0.** Not a defect — E-1 as written ("one `mdbook build docs` step closes it") is done, and that wording was mine and was optimistic about what the step catches. `mdbook-linkcheck` closes both cases and the new `docs` job is the obvious place to run it. Worth doing while the documentation surface is fresh, but it gates nothing shipping in 6.0.0 |
| **Pre-6.0.0 documentation gap: the release's two headline commands are undocumented as a workflow.** From `AUDIT-001.md` — **B-2** no guide for `get` or `set` (guides exist for `match-test` and for watching matches live; the two commands v6 exists for are the two without one); **B-3** `README.md` has **zero** occurrences of either; **B-4** the agent user is invisible outside the reference — *"agent"* appears on 2 of 42 doc pages, though RFC 048 § 1 calls U2 *"the user this release is for"*; **F-1** no example exercises `get` or `set`. Plus **E-1**, the docs build not being gated on PRs | **Handed off 2026-08-27 — [`rfcs/handoffs/pre-6-0-0-documentation/`](./rfcs/handoffs/pre-6-0-0-documentation/implementation-handoff.md); blocking the 6.0.0 cut.** Sequenced deliberately: E-1 first, because `ci.yaml` never runs `mdbook` and `docs.yaml` only fires on push to `main`, so the largest documentation change of the release would otherwise run against an ungated build — and the gate belongs in `ci.yaml`, not as a `pull_request` trigger on `docs.yaml`, which builds *and deploys*. The rest waits on RFC 065, which changes what `set --json` does. **Correction to F-1 as originally written:** it implied RFC 048's acceptance criterion was unmet. It is met — `crates/apimock/tests/set_w7_acceptance.rs` is a real acceptance test and CI runs it, satisfying *"v6 is finished when W7 is a script that runs in CI"*. W7 is **checked but not readable**; the item is to surface it as an example, not to rebuild the verification |
| **Duplicated logic is now a repeating defect source — three instances in the RFC 064/065 series, each found by someone tripping over it rather than by anything structural.** **(a)** RFC 064: three argument parsers (`args.rs`, `cmd/flags.rs`, `cmd/set.rs`), plus a fourth (`get.rs::positional_path`) found during Amendment 1 — dangling flags handled inconsistently, and `--allow-outside` sat behind the *un*shared copy. **(b)** RFC 065 D2: two custom-header orderings — RFC 045 fixed one site and the other silently discarded operator `content-type` headers. **(c)** RFC 065 § 5: `apimock-config`'s `respond_node_validation` is a second, independent implementation of `apimock-routing`'s `Respond::validate`. It did not learn about `json`, so `apimock validate` would have reported **a false error on every `set --json`-authored rule**. Found by the dev team testing the *success* path, not another failure case. The duplication in (c) is deliberate and justified — a GUI needs structured `(severity, message, target_id)` triples rather than `log::error!` plus a bool — but nothing keeps the two in step; the module's own "single source of truth" note describes `validate()`/`snapshot()` agreeing with each other, not with `Respond::validate` | **Architect's, after 6.0.0 unless it recurs before the cut.** Not a defect now — (a), (b) and (c) are each fixed. The concern is that the *mechanism* which produced them is intact. **Recommendation: a test running both validators over one shared corpus of `Respond` values, asserting they agree**, so divergence fails CI instead of waiting for the next feature to expose it. Cheap, and sufficient for the case that actually bit. A broader consolidation is not warranted — the (c) duplication has a real reason to exist |
| ~~**`apimock set rule --file=` (and `--file ""`) writes a rule set that then fails to load — at exit `0`, printing `Applied:`.** Found 2026-08-27 while reviewing RFC 064 Amendment 1. It writes `[rules.respond] file_path = ""`; the next `apimock validate` on the same workspace exits **2** with *"configuration validation failed"*. Precisely scoped — only `--file`: `--json=` and `--rule-set=` are rejected (exit 2), and `--text=` is legitimately valid. **Pre-existing**: the space form `--file ""` behaves identically and predates the amendment, which merely adds a second spelling reaching it.**~~ **RESOLVED 2026-08-27 by RFC 064 Amendment 1's § 3 fix, without being worked on directly.** | **Closed — no owner decision required.** I recommended it ride with the path-flag rule as "likely the same one-line fix"; it did, and went further. `reject_empty_path_value` sits at the *extraction point* after `flag_value` rather than inside the `=`-form parser, so it covers the space form too: `--file ""` now exits 2, writes nothing, and `validate` afterwards exits 0. Verified against the merged binary at `b62ab00`. Fixing where a value is *consumed* rather than where it is *spelled* is the third time in this RFC that choosing the shared layer paid off. Worse *in kind* than the dash-prefixed-value gap the amendment closes: that one was **loud** (exit 2), this is **silent** — `set` reports success and breaks the workspace, and the caller learns only when an unrelated later command fails on a config it never knowingly broke. This is [RFC 048](./rfcs/done/048-v6-cli-interface-concept.md) § 9's concern (`set` writing what it was not asked to) and the U2 failure mode exactly, on v6's headline feature. **Recommendation: reject an empty `--file` value at parse time**, alongside the path-flag rule now in RFC 064 Amendment 1 § 4 — likely the same one-line fix, and it should ride with that work rather than wait |
| **No CLI flag value can begin with `-`, and neither standard escape hatch exists.** Found 2026-08-27 while verifying RFC 064's § 5 (which correctly established only that there was no *regression*). Measured: `--text "- item"` → exit 2 *"unrecognized argument"*; `--text=-hello` → exit 2; `--text -- -hello` → exit 2 *"unknown option '--'"*. `reject_unknown_flags` (RFC 059) rejects any token starting with `-` before `flag_value` is reached, and there is no `--flag=value` form and no `--` end-of-flags separator. So `apimock set rule --text` cannot express a markdown bullet, a YAML `---`, or a diff hunk. Pre-existing across all of v5; **not** introduced by RFC 064 | **Approved 2026-08-27 — [RFC 064 Amendment 1](./rfcs/done/064-cli-front-door-completion.md), handed off; blocking 6.0.0.** Owner asked for a severe-risk check first; brief at `.git-exclude/reviewed/pre-6.0.0-audit/DECISION-001-flag-equals-value.md`. **One severe risk found, introduced rather than pre-existing:** `flag_present` discards any value, so a naive `=` split would resolve `--allow-outside=false` to "present" and disable RFC 062's write confinement while the author wrote `false` to keep it on. Today every `=` form is cleanly rejected, so a partial implementation is strictly worse than none — approved with two hard gates: no-value flags reject every `=` form (`=true` included), and `set.rs`'s private `reject_unknown_flags` is consolidated into the shared one, since the command carrying `--allow-outside` is the one not using it. A class below RFC 064's defects — it is *loud* (exit 2), not silent — but there is **no workaround at all**, and the message misnames the problem (*"unknown option '- item'; did you mean '-c'?"*). Weighs more in v6 than v5 because `set` is the headline feature and its intended author is a machine generating fixtures. **My recommendation: amend RFC 064** to add a `--flag=value` form — same files, same tests, same subject — rather than open a new RFC |
| **The CLI front door still mishandles two caller-supplied inputs.** Found in the pre-6.0.0 audit, 2026-08-27, and measured against a binary built from `9bdc769`. **(1)** A bare relative `-c apimock.toml` fails on every subcommand — exit 2, *"failed to resolve path"* — while `-c ./apimock.toml` works. `Path::new("apimock.toml").parent()` returns `Some("")`, not `None`, so `path_util.rs:19`'s guard never fires and `canonicalize("")` fails. RFC 049 already solved this for the root parser (`normalize_bare_relative_path`); the subcommands never picked it up, and RFC 057 worked around it a third time inside `set.rs`. **(2)** An *optional* flag given no value is silently defaulted — `flag_value` returns `Option`, so "present, no value" is indistinguishable from "absent". `set rule … -c` exits **0** and writes to a config the caller never named; a dangling `--format` returns human text, exit 0, to a caller that asked for JSON. Required flags are caught correctly and are not in scope | **Resolved — [RFC 064](./rfcs/done/064-cli-front-door-completion.md), merged to `main` 2026-08-27 (`9f9a193`), CI green on all 8 jobs.** Two of the three remaining RFC 048 § 6 prerequisites. Also corrects `docs/src/reference/cli-reference.md`, which documents (1) as **already fixed**, states the wrong exit code for (2), and argues in prose that (2) is unavoidable — a claim `reject_unknown_flags`'s own `no_value` parameter disproves. Worth recording separately: the conformance suite asserted, in two empty tests, that this scenario did not exist. RFC 059 asked for inapplicable scenarios to be stated rather than omitted, which was right; the claim written under it was never checked, by the dev team or by me |
| ~~**RFC 045 Goal 4 — `validate` passing on inert configuration — is still assumed, not established.**~~ **RETRACTED 2026-08-27 — the claim was false, and I made it twice without opening RFC 045.** RFC 045's Unresolved question 2 records Goal 4 as **✅ Resolved**: option 3 investigated *with a working prototype* and rejected because `Respond` and `DefaultRespond` share a field name (`delay_response_milliseconds`), so a name-based check passes while blind to the very defect RFC 045 existed to close; option 2 has no target (0 of 47 fields inert after the fixes); option 1 stands with reasons. RFC 045 asked for the outcome to be established rather than assumed and did precisely that | **Closed — no owner decision required; not blocking 6.0.0.** What RFC 048 § 6 left open was narrower — whether v6's machine-authored configs change the conclusion. Measured 2026-08-27: they do not, and the surface is *smaller* than in v5. An unknown key is **rejected at load**, under `[rules.guard]` and at `[service]` level alike. An agent cannot write a setting apimock silently ignores, because an unknown setting is an error. Retained struck through rather than deleted — the error was mine and reached the owner twice |
| 🔴 **Remotely reachable path traversal in `dyn_route`.** `curl --path-as-is 'http://host/../outside.txt'` returns **HTTP 200 with the file's contents** — verified 2026-08-20 against a running server. `dyn_route` joins a request-derived path onto the respond dir (`dyn_route.rs:126`) and checks only `.exists()`; `normalize_url_path` does not strip `..`, and there is **no traversal guard anywhere in `apimock-server`**. Encoded `%2e%2e` does not work; only a raw `..`. **Present in 5.19.0 and, by the code, every v5 release.** Found while scoping RFC 063 for the config-side sibling below | **BLOCKING for 6.0.0, and a shipped-software issue.** Bounded by the `127.0.0.1` default bind and by needing a client that does not normalise `..` — but bounded is not fixed, and "don't expose it" is not a control. [RFC 063](./rfcs/done/063-serve-path-confinement.md) drafted. **Owner decisions needed:** backport to a 5.19.1? publish an advisory? does 6.0.0 wait? My recommendation: backport and publish |
...
| **`apimock set --json` produces a `text/plain` response.** The value is stored as `respond.text` and served as `content-type: text/plain; charset=utf-8`. Not an implementation slip — `RespondPayload` exposes only `file_path`/`text`/`status`/`delay_milliseconds` (`view.rs:444`) and `Respond` has no JSON field either (`respond.rs:19`), so mapping `--json` to `text` is the only thing the `EditCommand` surface can express. Found 2026-08-19 reviewing RFC 057. **Confirmed end-to-end 2026-08-27** against a running server, which sharpens it: `set rule --path /api/user --json '{"id":1,"name":"ada"}'` exits 0, and `curl` returns the **correct JSON body** under `content-type: text/plain; charset=utf-8`. Body right, header wrong — so nothing looks broken until a client calls `.json()` or compares against the real API | **Resolved — [RFC 065](./rfcs/done/065-response-body-source-model.md), merged to `main` 2026-08-27 (`7d8a0d7`), CI green.** Owner chose the mechanism route over documenting it. Investigating turned up **three more defects in the same subsystem**, all from one cause — `respond` has no model of what kind of body it serves, so content-type is decided ad hoc at four sites: **(2)** an explicit `respond.headers.content-type` is silently overwritten on the `.json` path (`json_response` applies headers *before* `with_json_body`) — RFC 045 Defect 1b fixed the symmetric `text_response` site and not this one, so a vendor media type cannot be mocked; **(3)** `validate` **passes** on a config referencing a malformed `.json` file, which then 500s per request; **(4)** that 500 **echoes the absolute filesystem path** to the client — username and directory layout — where RFC 062/063 confined which files may be read, this discloses where they are. Note the cause of (1) precisely: `set --json` *already validates* its input and then stores it as `respond.text`, so the type is verified and then discarded by the schema |
| **Three of the four CLI commands accept unknown flags silently — the front-door rule is enforced only on `set`.** Verified 2026-08-20 against the built binary: `get /a --bogus` exits **0**; `validate -c cfg --bogus` exits **0** and prints "Validation passed"; `match-test /a --bogus` exits **1** for an unrelated missing-argument reason, not for the flag. Only `set` exits `2` (fixed by RFC 057). **The sharpest case: `validate --strct` — a typo of `--strict` — exits 0**, so a CI job that asked for strict validation silently gets non-strict and passes. Violates RFC 049's front-door rule and RFC 053's `usage`/exit-`2` contract, on the interface 6.0.0 is *about*. **Corrects an earlier row of mine** that recorded only `get` and stated `validate` rejects correctly — that came from testing `validate --bogus` with no config present, where it exits 2 for a different reason entirely | **Blocking for 6.0.0** in my view: U2 is the user that fails silently, and this is silence on the command CI runs. **[RFC 059](./rfcs/done/059-cli-contract-conformance.md) **accepted 2026-08-20**** — fixes all three and replaces the per-command tests that missed it with one cross-command conformance table |
