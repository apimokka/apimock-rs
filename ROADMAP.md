...
| **`respond.file_path` has no path-traversal protection at serve time.** A rule set with `respond_dir = "responses"` and `file_path = "../../outside.txt"` serves that file — reproduced 2026-08-20 against the built binary. **Not remotely reachable**: `file_path` values are static, operator-authored config, never built from request input. What it is: a config file can serve any file the process can read. Found by the dev team while writing RFC 062's threat-model page, reported rather than fixed as the handoff asked | **Decide before 6.0.0.** Matters more in v6 than v5 because v6 is *designed for machine-generated configs* — "don't run untrusted configs" is weak guidance for a tool whose headline feature is an agent writing them, and `set --file` is how such a value gets in. RFC 062 also leaves an unprincipled asymmetry: the write path is now confined, the read path is not. **RFC 063 recommended** — confine `file_path` under `respond_dir` with an opt-out, symmetric with RFC 062, or accept and document. Architect to draft |
| **A regression test I required cannot fail.** `save.rs::respond_dir_is_a_fixed_point_across_repeated_real_saves` — the guard I asked for in RFC 058's review — does one `Workspace::load` then three `apply`+`save` rounds on the *same* instance. RFC 058's growth needs a **fresh load** between saves, because the re-resolution happens in `RuleSet::new`, which only `load` calls; a same-instance loop can save forever and never observe it. So the test passes with the bug reverted. Found 2026-08-20 by the dev team while building RFC 060, whose first property had the same defect and was caught only because the acceptance ritual required proving it against the known bug | **Architect's — my error.** Add a fresh `load` per round. Small and isolated. RFC 060's idempotence property now covers the invariant correctly, so nothing is unguarded meanwhile; the test is simply weaker than its name. Worth one pass asking the same "can this test's *shape* fail?" question of other regression tests written against a bug the reviewer already knew how to trigger |
