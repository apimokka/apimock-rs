...
| Finding | Disposition |
|---|---|
| 🔴 **`npm-core-publish` failed on the 6.0.0 release — an npm propagation race in `release-publish.yaml`.** The step runs `npm install --package-lock-only` then `npm ci`, in a job that starts immediately after the three platform packages are published. **Unresolvable `optionalDependencies` are silently omitted from a generated lock**, so the lock was written without `@apimock-rs/bin-*@6.0.0` (not yet propagated), and `npm ci` then rejected it: *"can only install packages when your package.json and package-lock.json are in sync"*, `EUSAGE`. Left 6.0.0 partially published — three platform packages live, core not, crates.io skipped. `gh run rerun --failed` succeeded once propagation completed | **Architect's, before the next release.** Not a one-off: any release publishes the platform packages and then immediately locks against them, so the race is structural and 6.0.0 is simply the first time it lost. The same workflow **already knows about npm propagation delay** — `verify-published` retries `npm pack` six times over 90s — so the fix is the pattern the file already uses, applied one job earlier: retry the lock/`ci` step, or wait for the three platform versions to resolve before generating the lock. **Recovery is documented nowhere**; `RELEASING.md` § "If crates.io publishing fails partway" covers the crates.io case only, and should gain the npm one |
